...
这本书我还算比较熟悉，算是完整的看过一遍，部分章节看过两三遍，有些是校招准备面试的时候看看，真的挺合适的。

而这本书也被誉为计算机届的神书，豆瓣评分也是接近10分，个人感觉不至于神书，但是确实是一本适合程序员初步了解 System 知识的好书。
...
* 第一章是计算机系统漫游，核心就是讲清楚一个程序是如何编译运行起来的。
...
- 程序的优化这一块可以看下，循环展开、提高并行等，但是似乎作用不大
- 虚拟内存从内存管理和虚拟内存实现上讲的
- 程序的机器级表示这一章学过王爽《汇编语言》，所以看得还算比较快
...
[深入理解计算机系统高清版PDF](https://csguide.cn/resource/csapp.html)